...
// ─── Mock Data ────────────────────────────────────────────────────────────────
const USERS_SEED = [
  { id: 1, name: "Admin Singh", email: "admin@griha.com", password: "admin123", role: "admin" },
  { id: 2, name: "Rahul Sharma", email: "rahul@gmail.com", password: "user123", role: "user" },
];

const INIT_RECOMMENDATIONS = [
  { id: 1, title: "Modular Kitchen Upgrade", category: "Interior", roi: "18-22%", cost: "₹1.2L – ₹2.5L", description: "Install modular kitchen with granite countertops and stainless fittings. Hugely popular with buyers.", tags: ["Kitchen", "High ROI"] },
  { id: 2, title: "False Ceiling with LED", category: "Lighting", roi: "10-14%", cost: "₹40K – ₹80K", description: "POP/gypsum false ceiling with warm LED strips dramatically improves perceived value.", tags: ["Ambiance", "Low Cost"] },
  { id: 3, title: "Bathroom Makeover", category: "Sanitary", roi: "15-20%", cost: "₹60K – ₹1.5L", description: "Replace fittings, add glass shower partition, use anti-skid tiles.", tags: ["Sanitary", "High ROI"] },
  { id: 4, title: "Vastu-Compliant Entry", category: "Exterior", roi: "8-12%", cost: "₹20K – ₹50K", description: "Teak door, name plate, well-lit entrance with plants. First impression matters.", tags: ["Vastu", "Curb Appeal"] },
  { id: 5, title: "Smart Home Wiring", category: "Technology", roi: "12-16%", cost: "₹80K – ₹1.8L", description: "Pre-wire for smart switches, video doorbell, and automated curtains.", tags: ["Tech", "Future-Proof"] },
];

const INIT_PROPERTIES = [
  { id: 1, userId: 2, userName: "Rahul Sharma", type: "2BHK Flat", location: "Hyderabad, Kondapur", area: "1100 sqft", age: "8 years", issues: "Old flooring, dated bathroom, no modular kitchen", status: "reviewed", adminNote: "Priority: kitchen + bathroom. Estimated ROI after upgrades: 22%.", submittedAt: "2025-02-10" },
];

// styles moved to src/style/main.css
...
export default function App() {
  ...
  const [regUsers, setRegUsers] = useState([...USERS_SEED]);
  const [recommendations, setRecommendations] = useState(INIT_RECOMMENDATIONS);
  ...
  const handleRegister = (nu) => { setRegUsers(p => [...p, nu]); handleLogin(nu); };
  ...
  if (screen === "login")    return <LoginPage regUsers={regUsers} onLogin={handleLogin} onBack={() => setScreen("home")} onGoRegister={() => setScreen("register")} />;
      ...
      {showForm && (
        <div className="modal-backdrop" onClick={e => e.target===e.currentTarget && setShowForm(false)}>
          <div className="modal" style={{ maxWidth:560 }}>
            <h2 className="modal-title">New Recommendation</h2>
            <p className="modal-sub">Add a property improvement idea for homeowners</p>
            ...
            {/* Title dropdown — depends on category */}
            <div className="form-group">
            ...
            <div className="modal-actions">
              ...
              <button className="btn-sm btn-orange" style={{ fontSize:14, padding:"10px 24px" }} onClick={handleAdd}>Add Recommendation</button>
            </div>
          </div>
        </div>
      )}
...
function AdminProperties({ properties, setProperties }) {
  ...
  const handleReview = () => {
    ...
    setSelected(null); setNote(""); setSaved(true); setTimeout(() => setSaved(false), 3000);
  };
...
function UserProperties({ user, properties, setProperties, allProperties }) {
  ...
  const resetForm = () => { setForm({ type:"", city:"", area:"", size:"", age:"", budget:"", issues:"" }); setImagePreview(null); setImageData(null); setEditingId(null); };
  ...
  const handleSubmit = () => {
    ...
    if (editingId) {
      setProperties(allProperties.map(p => p.id===editingId ? {...p, ...form, location:form.city, image:imageData, status:"pending", adminNote:""} : p));
    } else {
      const np = { id:Date.now(), userId:user.id, userName:user.name, status:"pending", submittedAt:new Date().toISOString().split("T")[0], adminNote:"", image:imageData, location:form.city, ...form };
      setProperties([...allProperties, np]);
    }
    resetForm(); setShowForm(false); setSaved(true); setTimeout(() => setSaved(false), 3000);
  };

  const handleDelete = (id) => { setProperties(allProperties.filter(p => p.id!==id)); setDeleteId(null); };
...
function UserSaved({ recommendations }) {
  ...
  const savedList = recommendations.filter(r => saved[r.id]);
